...
SeekFail :: struct { exp: i64, seeked_to: i64 }
SizeMismatch :: struct { exp: int, given: int }
DMA_Error :: union { os.Error, SeekFail, SizeMismatch }

os_seek :: proc(fd: os.Handle, addr: i64) -> (err: DMA_Error = nil) {
  seeked := os.seek(fd, addr, 0) or_return
  if seeked != addr { err = SeekFail { exp = addr, seeked_to = seeked }; return }
  return }

os_read :: proc(fd: os.Handle, buf: []u8) -> (err: DMA_Error = nil) {
  tr := os.read(fd, buf) or_return
  if tr != len(buf) { err = SizeMismatch { exp = len(buf), given = tr }; return }
  return }

os_write :: proc(fd: os.Handle, buf: []u8) -> (err: DMA_Error = nil) {
  wr := os.write(fd, buf) or_return
  if wr != len(buf) { err = SizeMismatch { exp = len(buf), given = wr }; return }
  return }